...
const POOL_META: Record<string, PoolMeta> = {
  Milio: {
    ...
    wr: "54.1%",
    blindPick: "great",
    blindLabel: "Best blind in pool, OP tier",
    adcSynergy: [
      {
        name: "KogMaw",
  ...
  Sona: {
    ...
    wr: "53.2%",
    blindPick: "great",
    blindLabel: "OP tier, avoid engage matchups",
    adcSynergy: [
      {
        name: "MissFortune",
  ...
  Soraka: {
    ...
    wr: "52.1%",
    blindPick: "ok",
    blindLabel: "S tier, avoid engage matchups",
    adcSynergy: [
      {
        name: "KogMaw",
  ...
  Nami: {
    ...
    wr: "52.0%",
    blindPick: "ok",
    blindLabel: "S+ tier, bubble dependent",
    adcSynergy: [
      {
        name: "Lucian",
        reason:
          "E empowers his double-shot, bubble sets up burst combo perfectly. E cooldown buffed in 26.8, he'll be diving more often",
      },
      {
        name: "Vayne",
  ...
  Karma: {
    ...
    wr: "49.5%",
    blindPick: "ok",
    blindLabel: "A tier, nerfed twice, use carefully",
    adcSynergy: [
      {
        name: "Caitlyn",
  ...
  Seraphine: {
    ...
    wr: "51.5%",
    blindPick: "no",
    blindLabel: "S tier, still last-pick only",
    adcSynergy: [
      {
        name: "Ashe",
        reason:
  ...
  Lulu: {
    ...
    wr: "52.8%",
    blindPick: "great",
    blindLabel: "S tier, best peel + anti-engage",
    adcSynergy: [
      {
        name: "KogMaw",
        reason:
...
function getRecommendations(
  ...
  const recs: Recommendation[] = POOL.map((champ) => {
    ...
    if (!adcUnknown) {
      ...
      if (champ === "Nami") {
        if (allyAdc === "Lucian") {
          score += 25; // 26.8 buff: E CD reduced 18→16, mana cost down, he dives way more often
          reasons.push(
            "Nami + Lucian is the signature lane bully duo. E empowers every Lucian double-shot, bubble + E auto provides massive burst. Lucian's E was buffed in 26.8, lower CD and mana cost means he's diving more aggressively",
          );
          items.push({
            name: "Ardent Censer",
          ...
          tips.push(
            "Hit E on Lucian before every trade. His E buff means he'll dash in more, syncs perfectly with your bubble setup",
          );
        } else if (allyAdc === "Vayne") {
          score += 16;
          reasons.push(
  ...
  return (
    ...
    >
      ...
      <button
        ...
        className={`w-full cursor-pointer flex items-center gap-3 px-4 py-3 text-left transition-colors ${rank === 0 ? "bg-pink-50/70" : "bg-white hover:bg-pink-50/40"}`}
      >
        {/* rank badge */}
        <div
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5 flex-wrap">
            ...
            {/* mains badge */}
            {rec.champ === MAINS_CHAMP && (
              <span className="flex items-center gap-0.5 text-xs font-bold px-2 py-0.5 rounded-full bg-yellow-100 text-yellow-700 border border-yellow-200">
                ★ your main
              </span>
            )}
            {/* meta tier badge */}
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          {hasInput && (
            <button
              ...
              className="text-xs text-pink-400 hover:text-pink-600 underline underline-offset-2 cursor-pointer"
            >
              reset
            </button>
          )}
